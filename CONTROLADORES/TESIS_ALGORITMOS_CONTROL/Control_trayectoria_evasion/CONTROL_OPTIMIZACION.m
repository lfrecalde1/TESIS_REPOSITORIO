...
%     y_obj_min=[hy(k+1),valor_minimo(2)];
    ...
    %% almacenar datos de la trayectoria
    xdata=[xdata hx(k)];
                
    ydata=[ydata hy(k)];
    
    %% CREACIONES DE LA OBJETO PARA Q SE GRAFIQUE LA ORIENTACION DEL ROBOT
    shape1x=pose.Position.X-(0.05)*sin(phi(k+1));
    shape1y=pose.Position.Y+(0.05)*cos(phi(k+1));
    
    shape2x=pose.Position.X-(-0.05)*sin(phi(k+1));
    shape2y=pose.Position.Y+(-0.05)*cos(phi(k+1));
    
    %% PUNTOS DEL TRIANGULO DE ORIENTACION
    xdata_point=[shape1x  shape2x hx(k+1) shape1x];
    ydata_point=[shape1y  shape2y hy(k+1) shape1y];
    
    %% grafica del sistema para poder ver en tiempo real
